...
always_ff @(posedge clock) begin
    if(reset) begin
        ...
        //--------------for debug use-----------------//
        if(ex_packet_in[0].valid && ex_packet_in[0].pdest_idx != `ZERO_PREG) begin
            data[ex_packet_in[0].rob_idx].cdb_value <= `SD ex_packet_in[0].take_branch? ex_packet_in[0].NPC:ex_packet_in[0].result; 
        end

        if(ex_packet_in[1].valid && ex_packet_in[1].pdest_idx != `ZERO_PREG) begin
            data[ex_packet_in[1].rob_idx].cdb_value <= `SD ex_packet_in[1].take_branch? ex_packet_in[1].NPC:ex_packet_in[1].result; 
        end
        //-------------------------------------------//

        if(sq_fu_packet_in[0].done && data[sq_fu_packet_in[0].ld_rob_idx].rd_mem && sq_fu_packet_in[0].mem_violation) begin
            data[sq_fu_packet_in[0].ld_rob_idx].rd_mem_violation <= `SD sq_fu_packet_in[0].mem_violation;
        end

        if(sq_fu_packet_in[1].done && data[sq_fu_packet_in[1].ld_rob_idx].rd_mem && sq_fu_packet_in[1].mem_violation) begin
...
always_comb begin
    head_mispredict = 1'b0;
    if(data[head].ex_take_branch != data[head].predict_take_branch) begin
        head_mispredict = 1'b1;
    end else begin
        if( data[head].ex_target_pc != data[head].predict_target_pc) head_mispredict = 1'b1;
    end 
end
...
always_ff @(posedge clock) begin
    if(reset) begin
        ...
        if(data[head].halt && head_retire_rdy || data[head_p1].halt && head_p1_retire_rdy) begin
            system_halt <= `SD 1'b1;
        end else begin
            system_halt <= `SD system_halt;
        end
    end
end
...
always_comb begin
    ...
    if( (retire_tag[head] == 1'b1 || retire_rdy_indicator[head] == 1'b1) && !data[head].rd_mem_violation && !system_halt ) begin
        head_retire_rdy = 1'b1;
        n_retire_tag[head] = 1'b0;
        if( (retire_tag[head_p1] == 1'b1 || retire_rdy_indicator[head_p1] == 1'b1) && !data[head_p1].rd_mem_violation && !head_mispredict && !data[head].rd_mem_violation && !data[head].halt && !system_halt) begin
